...
**资金流 / 异动层**（`analysis/flow.py`，复刻文章作者 6/24 实战读法）：
- **今日异常活跃**（单快照即可）：近月、现价附近，`volume/OI` 高的行权价。**量 ≫ OI = 多为当日新建仓**，常在次日兑现为新 OI——是 ΔOI 异动的**当日先兆**。
- **日对日 ΔOI / ΔIV**（需 ≥2 天快照）：逐 (到期,行权价,C/P) 求持仓与 IV 变化，按 |ΔOI| 排序。新建 put（尤其 IV 同升）=看跌押注增强；新建 call=看涨。**叠加静态墙位**：新 OI 正堆在 put 墙上 + IV 升 = 作者那种自我实现的破位预警。
- **净倾向**只用**增建**侧聚合（方向无歧义）；put 减仓本身歧义（获利了结偏多/卖方撤退偏空），仅展示。
- ⚠️ 延迟数据无逐笔成交，方向是**启发式**推断，非成交主动性；CBOE 不存期权历史，**必须自己每天 `snapshot` 攒**。
