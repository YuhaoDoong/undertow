...
python3 -m undertow expiry                   # 近周到期阶梯：未来3周五+月度各自独立墙位+买卖方
python3 -m undertow expiry silver            # 定到期做价差用（如本周五 SLV 熊市看涨价差）
# —— 信号回测层（校准阈值）——
...
undertow/
  ...
  cli.py                    命令编排（analyze/gamma/snapshot/flow/expiry/vol/backtest/report/calendar/list）
  （每个子包 core/collect/analyze/report 各带一份 README.md，讲清该层职责与文件清单）
  ...
  analyze/                  【数据分析层】只吃 core.models，与数据源解耦，纯确定性计算
    ...
    expiry_ladder.py        ★ 近周到期阶梯：未来3周五+最近月度各自独立墙位+买卖方（定到期做价差用）
    macro.py                ★ 宏观背景：实际利率/美元/通胀预期 + 波动率指数
    volregime.py            波动率环境：期权偏贵/偏便宜 → 波段级买方/卖方倾向
    vrp_history.py          波动率溢价 VRP 跨周期检验（穿越牛熊，只落盘存档、不进日报）
    backtest.py             ★ 信号事件研究：无前视、发布滞后、对齐收益、分位分桶
    outlook.py              ★ 综合研判：多维因子按可信度加权投票 + 近端/中期双周期分层 + 关键位 + 情景
    strategy_hub.py         ★ 策略统筹：汇总各独立策略子模块 → 一张策略总纲
    strategy.py             ★ 方向性情景参数化（期货）：方向随研判、位点随结构、缓冲随 ATR、实时层否决票
    credit_spread.py        方向性信用价差子模块（偏空→熊市看涨价差 / 偏多→牛市看跌价差）
    condor.py               铁鹰子模块（区间震荡 + 偏卖方环境）
  report/                   【报告层】只吃 analyze 结果，只管展示（各文件详见 report/README.md）
    markdown.py             终端 Markdown 报告（COT / Gamma / 资金流 / 到期阶梯 / 回测）
    html.py                 ★ 自包含 HTML 研判报告（内嵌 SVG，浏览器直接看）
    viz.py                  ★ 手绘 SVG 图（价格+关键位 / OI 墙 / 持仓历史，零依赖）
tests/                      单元测试（不依赖网络，15 个文件 / 88 个用例）
...
**近周到期阶梯**（`analyze/expiry_ladder.py`，`expiry` 命令 + 自动嵌入 report）——服务**定到期的短线价差**：
- 主报告的墙/资金流是**把 60 天内所有到期揉进一格**看"整体墙在哪"；但做**某一个到期日**的价差
  （如本周五到期的 SLV 熊市看涨价差）时，要看的是**那一天单独的结构**。本模块把它拆回单个到期日。
- **选取**：未来 3 个周五（本周/下周/下下周）+ 最近的月度 OPEX（第三个周五）——周历差贴标签、锚真实今天。
- **每个到期各看各的**：对该到期**单独过滤快照**后，复用 `gamma`（call/put OI 墙、前三大墙、PCR）
  + `flow`（该到期昨→今独立 ΔOI 买卖方判定），口径与主报告完全一致。逐到期读数可截然不同——
  实测同一天近端两个到期：本周五"偏多·call 买盘"、下周五"偏空·卖 call 压制"。
- ⚠️ 逐到期买卖方需**两份工作日快照**（周末/盘前重复数据 OI 不动，那几条显示"待下一份点亮"，墙位始终有）；
  越近的周度到期 IV 噪音越大；仍是 ETF 代理位点，仅定性。
...
**方法论**：逐行权价「买方 vs 卖方」判读（OI 增减 × Delta 修正后相对 IV 方向 + 绝对 IV 闸门）、
垂直价差结构识别、波动率面「买方确认」检查、综合层「近端 vs 中期」双周期分层等规则，
均已落在 `undertow/analyze/`（flow / gamma / outlook 等模块），并与每日报告的方向结论逐条留痕、持续自校。
