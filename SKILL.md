...
| `python -m undertow shadow capture\|quote\|settle\|report` | **前瞻配对影子账（W05，只读，从不下单）**：检验「按墙选卖腿」是否比「不看 OI、按 ATR 距离选」多赚。每天盘前冻结每品种每侧的 A（局部最大墙）/ B1（1.0×ATR）/ B2（2.0×ATR）/ B3（卖腿 |Δ|≈0.20）四组价差（无候选也记原因，另记期限结构是否倒挂）；盘中两个窗口（ET 10:00–10:20、15:30–15:45）只追加两腿原始盘口尝试（失败可在窗口内重试，取第一次合格报价，不挑最好）；入场、持仓标记、1×/2× 权利金止损全部在结算时从原始观测派生，缺标记记为「路径未知」而不是「未触发」；所有臂同美元宽度（2×众数行权价间隔）。到期后按三种突破（窗末/期间收盘/盘中）与多口径损益结算：主终点「报价入场 + 到期内在值」，次终点含到期前一日收盘窗平仓、收盘越线次日开盘平仓、两种止损、快照模型。`report` 出 A 绝对收益与 A−B 配对差（循环移动块 bootstrap，主 5 日块、敏感性 10 日块；块数不足报样本不足；判定 支持/不支持/未决）。当前配置 shadow-v3-20260926。只有「已认证 session + 开盘前记录」计入主样本；`--as-of` 回放写 replay/ 不进主样本。**50 笔只是数据复核节点，不是放行线**。已接入每日任务 | `data/history/shadow/`（入 git） |
