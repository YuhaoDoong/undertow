...
| `python -m undertow shadow chain` | **开盘后近价全链快照（只读）**：每个交易日 ET 10:15–10:35（CBOE 延迟约 15 分钟，≈10:00–10:20 的报价，与影子账入场窗对齐）抓 15 个品种、≤14 天到期、现价 ±10% 内全部合约的 bid/ask/挂单量/IV/希腊值/成交，供以后在任意行权价上重做模拟；研究用报价，可成交性以 `shadow quote` 的长桥实时盘口为准。已接入 session_hooks（⑥），约 0.8MB/天 | `data/snapshots/options_open/`（入 git） |
| `python -m undertow shadow exec [--session D]` | **账户可执行账（S05，只读，从不下单）**：研究账有候选 ≠ 本账户可承受一组。读本账户净资产与现有持仓的分口径风险汇总，对当日每个候选推一组的收权金、费用、最大亏损、止损情景损失、所在簇与跨日占用（现有持仓 + 此前被判可执行且未到期的主臂候选），按 `analyze/risk_policy.py`（AGENTS.md 双层限额的唯一来源）给出「可执行 n 组 / 暂不可执行（原因）」；账户风险未知、净资产未知、无入场报价一律不可执行；单独买回短腿权限与到期处置列为未核实 | `data/account/shadow_exec/`（gitignore，含账户金额） |
