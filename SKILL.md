...
| `python -m undertow fib [品种...]` | **斐波那契回撤 + 盈亏比闸门**：确定性定位当前摆动腿 → 0.382/0.5/0.618 黄金回撤区 + 扩展目标，再算「现价追」vs「等回调」两情景的盈亏比并评级（差/中/优），把作者「先看盈亏比、别追、等回调」的交易哲学落成数字（含 ETF 行权价锚，实盘定腿用） | 终端 Markdown；也自动嵌入 `report` |
