...
| `python -m undertow shadow capture\|quote\|settle\|report` | **前瞻配对影子账（W05，只读，从不下单）**：检验「按墙选卖腿」是否比「不看 OI、按 ATR 距离选」多赚。每天盘前冻结每品种每侧的 A（局部最大墙）/ B1（1.0×ATR）/ B2（2.0×ATR）三组价差（无候选也记原因）；盘中 ET 10:00 抓两腿实时盘口作入场；收盘后监控「收盘越过卖腿」并在下一盘中窗口抓退出报价；到期后按三种突破（窗末/期间收盘/盘中）与多口径损益结算。`report` 出 A 绝对收益与 A−B 配对差（按日期分块区间，判定 支持/不支持/未决）。只有「已认证 session + 开盘前记录」计入主样本；`--as-of` 回放写 replay/ 不进主样本。**50 笔只是数据复核节点，不是放行线**。已接入每日任务 | `data/history/shadow/`（入 git） |
