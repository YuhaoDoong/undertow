...
> - Codex 008 全项目审查（2026-09-26）已处理：账户数据契约、私有档案严格存储、风险汇总与统一风控政策（`analyze/risk_policy.py` 唯一来源）、`shadow exec` 账户可执行账（私有）、实时体检日历、事件调度与只发布本目录、拉伸度同源口径与 COT 实际发布日、咨询 API 回环限制。
