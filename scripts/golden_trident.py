...
## 扩展检验（2026-09-04，用户问「是否最有效、多测一遍」）

### 参数敏感性（五品种 × swing 10~60，共 45 个配置）
复现：scripts/gt_param_sweep.py

  品种   买入持有   GT最好   GT最差   跑赢持有的配置数
  黄金    +157%    +118%    +46%      0/9
  白银    +250%     +89%    +18%      0/9
  原油    +100%     +76%     +9%      0/9
  纳指    +134%     +57%    +27%      0/9
...
**规律：趋势跟踪跑赢，当且仅当买入持有为负。**
四个上涨品种 36 个配置全部跑输；唯一下跌的美元，9 个里 8 个跑赢。
→ 它不是「择时工具」，是「空头保护」。价值只在标的下跌时体现。

参数还不稳定：原油 swing=20 是 +76%，swing=50 掉到 +9%。

### 子样本稳定性（前 500 / 后 500 根）
复现：scripts/gt_stability.py

  黄金 前 +15% vs 持有 +45%   后 +37% vs +78%
  白银 前  +3% vs 持有 +51%   后 +70% vs +138%
  原油 前  +2% vs 持有  −2%   后 +62% vs +106%   ← 唯一跑赢的那段，持有为负
  纳指 前 +17% vs 持有 +50%   后 +13% vs +60%

8 个子样本里只有 1 个跑赢，而那一段买入持有恰好是负的。
**结论稳定，不是时期依赖 —— 它就是一直跑输。**

### 与 Supertrend 同口径直接对比

           Golden Trident      Supertrend只做多        买入持有
  黄金     +98%(Calmar 5.5)    +40%(1.6)          +157%(5.9)
  白银     +75%(1.5)          +100%(2.5)          +250%(4.8)
  原油     +57%(1.7)            −2%(0.0)          +100%(3.1)
  纳指     +46%(3.6)           +61%(3.9)          +134%(5.8)

GT 比 Supertrend 好一些（黄金、原油上明显更好），但**四个品种全部跑不赢买入持有**，
Calmar 也是。

→ **答案：它是这几份脚本里设计最讲究的，但仍然没有能用的配置。**
