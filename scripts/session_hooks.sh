...
#
# 调度：plist 用 StartInterval=300（每 5 分钟轮询一次），脚本自己按【真 ET 时间】
# 判断在不在窗口。
#
# ⚠️ 为什么不用 StartCalendarInterval（2026-08-28 实测换掉的）：
#   原先挂了 15 个日历时点（EDT/EST 各一套）覆盖三个窗口。实测当天该响 12 枪，
#   launchd 几乎一枪没投——ET 09:42 的体检、ET 10:12 的事件后复核全部没跑，
#   而同机器上用 StartInterval 的 eventwatch(600s)/spreadlog(300s) 已分别稳跑
#   353/587 次零故障。密集的日历时点在 launchd 里不可靠，轮询可靠。
#   轮询还顺带解决了夏令时：脚本读真 ET，plist 里再不用挂两套时点。
#
