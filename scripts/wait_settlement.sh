...
# 所有存活合约一张没动，总量差全来自到期滚出）。
#
# ⚠️ 2026-08-30 实测修正：**周五收盘的 OI，周六周日都拿不到，要等周一凌晨。**
#   8/21(五)→8/22(六) Σ|ΔOI|=0、8/22→8/23(日)=0、8/23→8/24(一)=892,751。
#   我此前说"历史上两个周六 ET 07:00/08:00 拿到过"是**用错了口径**
#   （看的是 OI 总量差，那个会被到期滚出污染）。
#   所以周末跑这个脚本基本注定空转，真正有用的时段是周一凌晨。
set -uo pipefail
cd "$(dirname "$0")/.." || exit 1
