...
现行：dir-analysis-v1.3（2026-09-26，Codex 013）。v1、v1.1、v1.2 已被取代：它们的代码守卫退役，
但各版的文本与 JSON 必须保持原样（哈希记在 v1.3 JSON 的 supersedes 段），被取代的旧判断不静默覆盖。
"""
import hashlib
import inspect
import json
...
PREREG = ROOT / "docs" / "prereg" / "2026-09-26_direction_v1.3.json"
...
def test_superseded_v1_documents_preserved():
    sup = _doc()["supersedes"]
    assert sup["version"] == "dir-analysis-v1.2-20260926" and sup["real_samples_seen"] == 0
    assert len(sup["sha256"]) == 6           # v1、v1.1、v1.2 的文本和 JSON
...
def test_v1_and_current_share_design_constants():
    """v1.1–v1.3 不改设计常数：v1 冻结的常数必须原样保留（version/supersedes 以外）。"""
