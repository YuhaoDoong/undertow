...
| `shadow_direction.py` | **方向次要分析（预登记 dir-analysis-v1.1，取代 v1；docs/prereg/2026-09-26_direction_v1.1.md）**：在 v5 两侧记录上检验 H-dir = Y(B1,顺)−Y(B1,逆) 与 H-wall|dir = Y(A,顺)−Y(B1,顺)，单侧 α=0.05/14、经济门槛 δ=0.03（未校准）。准入在模块内（`eligibility`：session 认证、aware 时间 available_at < recorded_at < 09:30 ET、快照/台账哈希、映射版本、台账方向算法指纹），`opportunity_table` 按日历交易日×品种逐格分类（成熟缺结果留在分母），`common_sample` 只在共同键上比较顺/逆/固定卖 put/call，`diagnostics`（dir-diagnostics-v1）描述开仓前/后按方向取号的标的移动。判定区分「未证实超过门槛」与「排除实用增量（上界<δ）」。依赖函数指纹冻结在预登记 JSON，由 tests/test_prereg_freeze.py 守卫。 |
