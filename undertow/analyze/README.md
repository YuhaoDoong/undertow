...
| 文件 | 作用 |
|---|---|
| `gamma.py` | OI 墙（阻力/支撑/pin）· Put-Call 比 · 做市商 GEX 正负 · 零伽马翻转位。<br>OI walls, P/C ratio, dealer GEX, zero-gamma flip.<br>2026-09-25 加 `local_wall()`：近价局部墙（≤14 天、±5% 内该侧累计 OI 最大档），与 `pick_sell_wall` 的结构主墙分开 —— 用户实际卖的是前者。 |
...
| `risk_aggregate.py` | **账户风险汇总（分口径、带完整性）**：最大亏损（裸卖 call 无上限、裸卖 put 按接货全额、跨期结构算不出）、止损情景、资金占用、有符号 Greeks 分开；任一未知成员 → 合计 None + 已知部分 + 缺失清单；限额 pass/fail/incomplete，incomplete 不是通过。account 命令与咨询包都显示（Codex 008 G03）。 |
| `shadow_exec.py` | **S05 账户可执行账（纯计算）**：对影子账候选推一组的经济量与可开组数，跨日占用只累计主臂，到期处置未核实项照实列出；与公开的研究账严格分开，输出只写 gitignore 的 data/account/。 |
| `shadow_direction.py` | **方向次要分析（预登记 dir-analysis-v1.3，取代 v1.2/v1.1/v1；docs/prereg/2026-09-26_direction_v1.3.md）**：在 v5 两侧记录上检验 H-dir = Y(B1,顺)−Y(B1,逆) 与 H-wall|dir = Y(A,顺)−Y(B1,顺)，单侧 α=0.05/14、经济门槛 δ=0.03（未校准）。准入在模块内，顺序为 session → 信号来源 → 方向（`eligibility`：session 认证且 recorded_at < 09:30 ET；两份来源快照抓取时刻都已知且早于 recorded_at、available_at=二者较晚者；快照/台账哈希、映射版本、台账方向算法指纹；来源不合格的中性行记身份失败而非弃权），结果只在 as_of 晚于退出日时读取，`opportunity_table` 按日历交易日×品种逐格分类（成熟缺结果留在分母），`common_sample` 只在共同键上比较顺/逆/固定卖 put/call，`coverage`/`scope` 与冻结的条件配对率并列报告全日历覆盖和结论适用范围（记录完整性与收益可评估性分开；入场缺报价/未定价/无界/有限区间都算收益未完整识别，推广判未决，不是闸门），`common_bounds` 给共同可界定键上的计价包络与 [Lx−Uy, Ux−Ly] 相对区间，`diagnostics`（dir-diagnostics-v1）描述开仓前/后按方向取号的标的移动。判定区分「未证实超过门槛」与「排除实用增量（上界<δ）」。依赖函数指纹冻结在预登记 JSON，由 tests/test_prereg_freeze.py 守卫。 |
| `shadow.py` | **前瞻配对影子账的纯计算层**（W05/W06；Codex 004 蓝图、005/006 审查）：预登记配置 `CONFIG`（版本+hash，改动即新版本）。v5（Codex 007：零买价需有卖价与卖量才算有效报价；只买回短腿时残腿按到期结果给点值或 [None, 上界]，`pnl` / `pnl_bounds` / `pnl_residual0` 分存；`interval_ci` 按 [A下界−B上界, A上界−B下界] 传播；`paired_summary` 出 A全体/A配对/B配对三均值，estimate=bounds/both_legs_only/residual0；`opportunity_ledger` 按池×品种×侧×交易日生成分母；`metrics_table` 描述性指标）。v4：A=墙选腿、B1/B2=按 ATR 距离、B3=按卖腿 |Δ|≈0.20；主比较 A vs B1；**唯一主终点 `pre_expiry_exit_policy`**（到期前最后交易日收盘窗的退出政策，入场日本身可以是这一天），到期内在值等为副终点；品种分三池（etf / leveraged_etf / single_stock），分池报告、从不合并。所有臂同美元宽度（2×众数间隔，保护腿不在网格 → 不可配对）。账本只存原始盘口尝试；`window_leg` 按预登记质量规则派生入场/标记，出场要求两腿都合格（保护腿报错/缺失/零量 → 缺失；保护腿有效报价 bid=0 → `short_only`，残腿按到期结果给点值或区间（v5））。窗口与应有观测点来自 `core.market_calendar`（收盘窗 = 核心收市前 30~15 分钟，半日市 12:30–12:45），缺日线记 `bars_incomplete`，不当休市。`settle_leg` 各终点独立成熟（`immature`），止损四态。`block_bootstrap` 沿日历交易日做循环移动块（5 日主/10 日敏感性），有样本日期 // 块长 ≥4 才出区间，常数样本或零宽区间标 `degenerate` 不判定；2026-12-31 之前一切判定标「探索」；另有不重叠入场的敏感性。无 I/O，编排在 `undertow/shadow_cli.py`。 |
...
| `livecheck.py` | **持仓实时体检**：把长桥实时盘口翻成「现在就走能拿回多少」——多头腿按 bid 卖、空头腿按 ask 买回。<br>⚠️ **券商 App 的持仓盈亏用 last 价，对流动性差的腿会系统性高估**。2026-08-26 TQQQ 76/80 同一时刻三个口径：App(last) $100(+$10) ／ 中价 $85(−$5) ／ **真实可平仓 $67(−$23)**，差 $33 = 成本的 37%（因 76C 盘口 1.37/1.67 宽达 20%，而 last 1.68 正是自己的成交价）。**止损判定必须用可平仓价**，看 App 会系统性晚动手。<br>单边盘口缺失时返回 None 而非用 last 顶替。<br>**另含品种累计台账**：只吃现金流水，不接触券商「成本价」——后者在部分减仓后会被改写成该轮打平价（实测 SLV 实付 1.05、券商显示 1.89 =(3×1.05−2×0.63)/1，是把已实现亏损摊进了剩余持仓）。台账按【当前持有的具体合约】聚合，不按品种（按品种会把该标的其它已了结仓位算进来）。三个数各回答一个问题：已实现(沉没·决策时无视) ／ 可平仓(唯一影响当下决策) ／ 累计(复盘用)。CLI：`undertow live`<br>Real closeable value from live L2 depth — App's last-based P/L systematically overstates. |
