...
def probe_strong_signal(fa: "FlowAnalysis") -> dict:
    ...
    out: dict = {
        ...
        **concentration_stats(fa),
        ...
        # 预注册假设的观测量：逆向绝对量（比值的分母）。分母小时比值会爆炸而失真
        # —— 2026-08-12 黄金 360.8× 的分母只有 694。只记录，不参与判定。
        "reverse_abs": round(min(up, dn), 1),
        "forward_abs": round(max(up, dn), 1),
...
#
#   ⚠️ 周末例外（2026-08-30 实测，两个周末一致）：
#     **周五收盘的 OI 结算，周六和周日都拿不到，要等周一凌晨。**
#       8/21(五)→8/22(六)  Σ|ΔOI| = 0
#       8/22(六)→8/23(日)  Σ|ΔOI| = 0
#       8/23(日)→8/24(一)  Σ|ΔOI| = 892,751  ← 周一凌晨 ET 04:09 才到
#     8/28(五) 这一周完全相同：周六守到 ET 07:42、周日 ET 00:37 均为 0。
#     所以「周一早上的研报」实际描述的是【上周五】的交易日，中间隔了两天。
#
...
def analyze_flow(
    ...
    *,
    ...
    if prev is not None:
        ...
        # ── 预注册假设（2026-08-30，用户追问 360.8× 时发现，**尚未验证**）──
        # 判断信号可信度，也许该看【逆向绝对量】（比值的分母）而不是比值本身。
        #
        # 三种度量按自身分三档的区分度实测（43 个正确去趋势样本，1 日窗口）：
        #     比值 ratio        低 86% / 中 64% / 高 67%   ← 反向且不单调
        #     不平衡度          低 64% / 中 86% / 高 67%   ← 不单调
        #     逆向绝对量        低 57% / 中 64% / 高 93%   ← **唯一单调**
        # → 分母问题的解法不是"修比值"，而是"别拿比值当主指标"。
        #   比值把两件事（方向倾斜程度 + 证据厚度）挤进一个数，还在分母小时爆炸。
        #
        # 前瞻窗口实测（用户 2026-08-30 提出"不该只看当日"）：
        #     1日 72%(p=0.005) / 2日 57% / 3日 65% / 5日 48% / 10日 55%
        # → 增仓是"昨天的变化量"，信息只对次日有效，2 日起就衰减。
        #   用户举的 8/12→8/13 是个案：8/12 信号日 +0.99%、8/13 −1.47%，
        #   但 8/14 +0.63%、8/17 +1.00%、8/19 +3.84%，3 日起累计转正。
        # 事后分档实测（43 个正确去趋势样本）：
        #     分母 <1,000      5 个  命中 40%
        #     分母 1,000~5,000 21 个 命中 62%
        #     分母 ≥5,000      17 个 命中 94%（日聚类 95% 区间 [78.6%, 100%]）
        # 机制上说得通：两边都在大量建仓、其中一边明显更多，比"一边几乎没人"更可信 ——
        # 后者可能只是流动性缺失造成的假象，而比值恰恰在那种时候最大。
        # 2026-08-12 黄金 360.8×（分母 694）当日 +0.99% 判错，就是这种情形。
        #
        # ⚠️ 绝不可据此下结论：只有 11 个日聚类（远低于 50）、
        #    17 个样本高度集中在金银（相关 0.89）、且这是【事后分档】——
        #    我试了比值分档和绝对量分档，挑了看起来有区分度的那个。
        #    这里只做 shadow 记录，等新样本检验。
        _rev_abs = min(upside, downside)      # 逆向绝对量，仅记录不判定
