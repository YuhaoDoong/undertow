...
def structural_walls(snap, today: date, spot: float, kind: str, *,
    ...
    **不保证权利金可观**：结构墙可能距现价很远（SLV 50 距 −14.5%），
    ...
    min_dte —— 剩余期限下界（天）。
        0（默认）只剔已过期的合约（数据滞后残留）。报告展示用这个 ——
          当日到期的堆积可能与当日盘中的对冲流有关，值得看见。
    ...
    ⚠️ 为什么要有 min_dte（2026-09-03 用户问「黄金之前有 408 的墙，
       ...
       这种堆积**可能**影响当日的 pin 与做市商对冲流，但仅凭 OI 无法确证；
       能确定的只有一件事：它收盘后消失，**不能支撑跨日的墙位**。
...
#   ⑤ 若最大墙自身缓冲也不足门槛 → 弃权
#      缓冲门槛统一 3%（2026-09-02 在 GLD 上验证后定）。门槛敏感性：
#                   门槛2%            门槛3%            门槛4%
#          GLD put  77%/3.0%/4.3%   67%/0.0%/5.7%   60%/0.0%/7.1%
#          GLD call 86%/2.7%/6.3%   79%/0.0%/7.0%   74%/0.0%/7.2%
#          SLV put  98%/0.0%/7.1%   98%/0.0%/7.1%   91%/0.0%/7.4%
#          SLV call 88%/2.6%/8.3%   84%/0.0%/8.9%   81%/0.0%/9.8%
#                （覆盖率 / 破墙率 / 均缓冲）
#      3% 是四个组合的共同最低点：全部 0% 破墙，再往上只丢覆盖率。
#      ⚠️ 黄金天然吃亏：行权价间隔 5 美元 ≈ 现价 1.25%，白银 1 美元 ≈ 1.8%。
#         档位越密，「上挪一层」挪的距离越小，越容易撞门槛而弃权 ——
#         GLD put 覆盖率 67%，SLV put 98%。
#
# 实测（43 个可交易日，2026-06-26~09-01，门槛 3%，含退回规则）：
#     SLV put   覆盖 98%  破墙 0.0%  均缓冲 7.1%
#     SLV call  覆盖 84%  破墙 0.0%  均缓冲 8.9%
#     GLD put   覆盖 67%  破墙 0.0%  均缓冲 5.7%
#     GLD call  覆盖 79%  破墙 0.0%  均缓冲 7.0%
# 唯一一笔在门槛 2% 下的破墙是 GLD 2026-09-01：卖 400 put、缓冲 2.1%，
# 前收 408.42 → 当日收 396.75，破 3.25。门槛提到 3% 后该笔被挡在门外。
#
# ⚠️ 样本期 SLV +9.7%，put 侧的 0% 含方向成分；call 侧是逆势方向，
#    那个 0% 相对更有说服力，但代价写在缓冲里（10.4% 意味着权利金极薄）。
#    未经跌市验证。
WALL_PICK_MAX_DIST = 0.25       # 三道墙的取用半径
WALL_PICK_MIN_BUF = {"P": 0.03, "C": 0.03}
