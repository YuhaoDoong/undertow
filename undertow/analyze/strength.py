...
#   —— 所以【跨品种合并】的结论必须撤回：Flow/Vol/Price 三层都没验证通过。
#
# ⚠️ 但撤回不该过度（用户 2026-08-29 质疑后修正）：日聚类修正是为跨品种同日
#    相关设计的，**对单品种时序不适用**。同品种不同日、前瞻 1 日、窗口不重叠，
#    二项检验才是对的工具。按它算：
#      黄金 23/32=72%(p=0.0201, 下界 54.6%)、白银 20/28=71%(p=0.0357, 下界 52.9%)
#      原油 16/30=53%(p=0.86)、QQQ 7/12=58%(p=0.77)
#    黄金大波动日单独看 9/11=82%。**这是事实，不能因为合并结论撤回就一并否认。**
#    仍够不上"已验证"：Bonferroni 校正后阈值 0.0125（黄金 0.020 过不了）、
#    n=32<50、只有两个月。定性：比探索性观察强，够不上已验证。
#
...
CAL = {
    ...
    # ⚠️ 上限 20× 的饱和在这里恰好起了保护作用：2026-08-12 黄金 360.8×
    # （因分母只有 694）与 8/28 的 53.5× 被压成同一个强度值 1.0。
    # 但**展示层没有这个保护** —— index 上一度只报「看跌侧 360.8×」，
    # 会让人以为它比 53.5× 强得多。已改为倍数与绝对量并列。
    "flow": (1.3, 20.0),
