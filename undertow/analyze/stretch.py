...
**已知边界（回测实测，不是猜的）**
  ...
  * **只有超卖侧可用，超买侧一格都不可用**。判定统一由 reliability() 给出（检验 n≥50、|t|≥2、
    t 与边缘同号，三者来自同一批不重叠样本）；具体 t 与 n 读 CALIB，不在散文里写死。
    2026-09-26 同源口径重算（Codex 008 G06）后结论不变：显著的四格全在「极/强超卖 × 牛/熊」；
    超买侧唯一 |t|≥2 的「强超买-熊」检验样本量只有 36，判「样本不足」。
    ⚠️ 同源口径下「极超卖」与「强超卖」的边缘已不单调（区间大幅重叠），不能说「越深越好」，
    只能说两档深度超卖都高于偏超卖与中性。旧→新对照：data/history/stretch_calib/。
    超买只能读作「追高性价比差」，不能读作「要反转」。

  * **准确率要看清是哪一种**。超卖档"跑赢什么都不做"的比率 60~69%（基准 47~58%），
    但**绝对方向准确率只有 56~60%（基准 55~56%）——提升仅 1~5pp**。前者高是因为
    它扣掉了局部漂移；别把 65% 当成"六成半的时候会涨"。幅度上 5 日约 +1.1~1.2pp（不重叠口径，见 CALIB），
    **对期权买方而言这个量级很容易被点差与 theta 吃光**。

  * **两维分歧时边缘明显变弱**。2026-09-26 同源口径（边缘与 t 同为不重叠样本）：
        两维都超卖（一致）    +1.04pp  t=+3.64
        回撤深但偏离不深      +0.66pp  t=+1.71（不显著）
        偏离深但回撤不深      +0.04pp  t=+0.07（不显著）
        两维都超买（一致）    −0.33pp  t=−1.54（不显著）
    （触发与检验 n 读 `backtest-stretch` 输出；散文不写样本量。）
...
#   2026-09-26 起（Codex 008 G06）边缘、跑赢中性桶%、t 全部来自同一批不重叠子样本；
#   旧→新对照见 data/history/stretch_calib/2026-09-26_G06_old_vs_new.md
# 数值单位：百分点(pp)。改任何指标参数后必须重跑本表，别手改。
CALIB_ASOF = "2026-09-26"
...
CALIB_META = {
    ...
    "n_total": 29582,          # 全部样本（重叠）
    "n_test_total": 5918,      # Welch t 实际用的不重叠子样本合计
    "mode": "combo",           # 两维分位均值
    "horizon": 5,
...
_CALIB_RAW = """
极超卖 牛 +1.063 66 472 3.18 103
强超卖 牛 +1.194 73 577 3.24 109
偏超卖 牛 +0.249 57 2265 1.34 457
中性 牛 +0.000 55 10308 0.00 2077
偏超买 牛 -0.181 52 3636 -1.36 684
强超买 牛 -0.427 49 1232 -1.71 237
极超买 牛 -0.225 47 1024 -1.12 222
极超卖 熊 +1.114 61 766 2.31 155
强超卖 熊 +1.096 55 781 2.08 146
偏超卖 熊 +0.306 54 2167 1.12 437
中性 熊 +0.000 51 5173 0.00 1054
偏超买 熊 -0.189 46 831 -0.60 171
强超买 熊 -1.030 25 202 -2.09 36
极超买 熊 -1.251 40 148 -1.70 30
"""
...
def reliability(t, n_test, edge) -> str:
    """唯一的可靠性判据（Codex 008 G06：所有输出共用，不各写一套）。

    显著 = 检验样本量 ≥ MIN_TEST_N、|t| ≥ T_SIGNIFICANT、且 t 与边缘同号（二者须来自同一批不重叠样本）。
    旧版回测表只看 |t|：检验 n=3 也标「✅ 显著」，边缘 +4pp 配 t=−2.45 也标显著。
    """
    if t is None or n_test is None or edge is None:
        return "无数据"
    if n_test < MIN_TEST_N:
        return "样本不足"
    if t != 0 and edge != 0 and (t > 0) != (edge > 0):
        return "方向矛盾"
...
def analyze_stretch(series) -> StretchRead:
    ...
    reliable = reliability(t, n_test, edge) == "显著"
...
def _n_test(sr) -> int | None:
    cal = CALIB.get((sr.band, sr.regime))
    return cal[4] if cal else None
...
def render_md(sr: StretchRead, display_name: str = "") -> str:
    ...
    if sr.edge_pp is not None and sr.band != "中性":
        flag = "" if sr.reliable else f"（{reliability(sr.t_stat, None if sr.t_stat is None else _n_test(sr), sr.edge_pp)}，t={sr.t_stat:+.2f}，参考级）"
