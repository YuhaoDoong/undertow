...
**已知边界（回测实测，不是猜的）**
  ...
  * **只有超卖侧稳定过得了显著性**。14 格里 5 格 |t|≥2，其中 4 格在超卖侧
    ...
    超买侧只有"强超买-熊"过线，且 n 只有 202；其余方向对、大体单调，但 t 在
...
CALIB: dict[tuple[str, str], tuple[float, float, int, float]] = {}   # 由 _CALIB_RAW 填充
_CALIB_RAW = """
极超卖 牛 +1.364 65 471 3.16
强超卖 牛 +0.725 60 574 3.25
偏超卖 牛 +0.369 55 2250 1.36
中性 牛 +0.000 47 10274 0.00
偏超买 牛 -0.340 41 3626 -1.31
强超买 牛 -0.293 41 1225 -1.81
极超买 牛 -0.416 40 1021 -1.17
极超卖 熊 +1.501 69 766 2.30
强超卖 熊 +0.752 64 781 2.07
偏超卖 熊 +0.121 58 2167 1.10
中性 熊 +0.000 58 5138 0.00
偏超买 熊 -0.426 52 829 -0.65
强超买 熊 -0.519 53 202 -2.10
极超买 熊 -1.061 49 148 -1.71
"""
for _ln in _CALIB_RAW.strip().splitlines():
    _b, _r, _e, _w, _n, _t = _ln.split()
    CALIB[(_b, _r)] = (float(_e), float(_w), int(_n), float(_t))
...
def analyze_stretch(series) -> StretchRead:
    ...
    if band == "中性":
        ...
        head = (f"{band}·{regime}市（合并分位 {pct*100:.0f}%）—— 历史上此后 5 日跑赢"
                f"什么都不做 {edge:+.2f}pp，跑赢率 {wr:.0f}%（n={nh}, t={t:.2f}）")
    else:
        head = (f"{band}·{regime}市（合并分位 {pct*100:.0f}%）—— 历史上此后 5 日跑输"
                f"{edge:+.2f}pp（n={nh}, t={t:.2f}）；"
                f"**是「追高性价比差」，不是「要跌了」**")
...
def render_md(sr: StretchRead, display_name: str = "") -> str:
    ...
    if sr.edge_pp is not None and sr.band != "中性":
        ...
        L.append(f"- 回测校准（{CALIB_ASOF}）：5 日边缘 **{sr.edge_pp:+.2f}pp**、"
                 f"跑赢率 {sr.win_rate:.0f}%、n={sr.n_hist}{flag}")
    L.append("")
    L.append("> 口径：边缘 = +5日收益 − 过去60日局部漂移 − 同 regime 中性桶，即「比什么都不做多赚多少」。")
    L.append("> 「跑赢率」不是「上涨概率」：超卖档绝对方向准确率仅 56~60%（基准 55~56%）。")
