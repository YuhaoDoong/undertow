...
REGISTRY: dict[str, Validation] = {
    ...
    "ta_indicators_direction": Validation(
        key="ta_indicators_direction", label="技术面子模块的方向有效性（按品种拆）",
        n=2733, hits=1328, p_value=None, baseline=0.539, cluster_n=397,
        note="复现入口：scripts/validate_ta_by_symbol.py。四品种×三周期(1h/4h/1d)，"
             "信号只用第 i 根及之前的数据算出，测 close[i+3]/close[i] 的方向。\n"
             "**基线不是 50%**：样本期整体上涨，各品种一直做多的命中率就是"
             "黄金 54.1% / 白银 53.6% / 原油 55.1% / 美元 52.9%。\n"
             "按连续同向段聚类 bootstrap（4000 次），**按品种分别检验**：\n"
             "  金银原油 15 个(指标×品种)组合 —— **全部与基线无区别**\n"
             "  美元 UUP 5 个指标 —— **全部显著劣于一直做多**\n"
             "    UT持仓 42.3%(基线52.9) · Stoch 43.3% · MACD 44.2% · "
             "DI方向 45.9% · ST趋势 48.0%\n"
             "事件型样本太小：ST翻转 n=100、MACD交叉 n=192、DMI回调入场 n=37，全部不显著。",
        caveat="⚠️ **合并样本会得出错的结论。** 先前把四品种×三周期的段混在一起，"
               "得到『UT持仓/Stoch/DI方向 三个显著劣于』；按品种拆开后发现"
               "**那三个『显著』完全是美元一个品种拖出来的**，金银原油上全是无区别。\n"
               "这与 vol_surface_as_filter 那次是同一个错误（四品种平均 47% 是 USO/QQQ 拖的）。"
               "**统计必须按品种拆** —— 金银高度相关、1h/4h/1d 是同一段行情的不同切片，"
               "合并后名义段数虚高，CI 过窄。\n"
               "⚠️ **聚类校正也是必须的**：状态型指标相邻根的状态几乎相同"
               "（一段趋势能连着几十根）。未校正时六个指标全部『显著』、五个 p<0.01；"
               "校正后合并口径只剩三个，再按品种拆只剩美元那一列。\n"
               "⚠️ **『无区别』是功效不足，不是证明无效。** 单品种段数 21~107，"
               "bootstrap CI 宽 11~13pp，标准误约 3pp —— 只能检出 **≥5~6pp** 的效应。"
               "真实效应若在 2~4pp，这个样本量看不见。\n"
               "⚠️ 适用范围：只对这段以涨为主的行情成立。趋势指标在回调时翻空，"
               "而这段行情的回调都被快速买回。不代表震荡市或下跌市无效，但我们没有那样的样本。\n"
               "⚠️ 2026-09-04 用户拿 TradingView 的 Supertrend 策略回测追问后，"
               "在四年长历史(2022-09→2026-09，1000 根日线)上复盘，结论一致且更清楚：\n"
               "  GLD 买入持有 +164.8% vs Supertrend 满仓 −28.7%（0 成本）\n"
               "  SLV 买入持有 +253.3% vs Supertrend 满仓 +13.9%\n"
               "拆开看：做多段胜率 57~61% 贡献为正，**做空段胜率只有 18~22%**，"
               "把利润全吃掉。做空段按方向计平均 −3%，意味着那些『红色段』里"
               "价格其实平均涨了 3% —— 牛市里 Supertrend 的红色段不是下跌段，只是回调。"
               "复现：scripts/supertrend_long_horizon.py\n"
               "→ 结论：ta 包维持『不进研报、不进方向投票』。"
               "这不是因为证明了它们无效，而是**没有任何证据表明它们有效**，"
               "且在美元上明确有害。"),
