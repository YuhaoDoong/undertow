...
REGISTRY: dict[str, Validation] = {
    ...
    "thin_denominator_gate": Validation(
        key="thin_denominator_gate", label="薄分母（逆向压力<1000）是否该压制强信号",
        n=27, hits=11, p_value=0.4421, baseline=0.5, cluster_n=None,
        note="复现入口：直接读 data/history/signals/*.json 的 up_pressure/dn_pressure，"
             "lo_side=min(两者)，与 forward_5d（局部去趋势：减 drift_60d×5）对照。\n"
             "起因：一份外部独立判读认为 2026-09-04 黄金的期权结构以噪音为主、"
             "大资金处于观望，而我们同日给了 ⚡强看跌 496.1×。\n"
             "那一天的比值确实是分母塌出来的：看跌侧 403,494 vs 看涨侧 813，"
             "而 813 是 churn_call=0.27 折减后的结果（原始 call 净建仓 45,613 手）；"
             "whale_like 由 9/3 的 True 转 False，flip_k=98/n_legs=192。"
             "次个交易日（9/7 抓到的 9/4 收盘链）491.5×→4.69×、净Δ −20,133→−1,490，"
             "**证实那是非农事件对冲而非方向性建仓** —— 就这一例而言他是对的。\n"
             "但推广成规则则不成立。27 个已回填的开火样本按分母厚薄拆：\n"
             "  薄分母 lo<1000    3/8  =37.5%  p=0.7266\n"
             "  厚分母 lo>=1000   8/19 =42.1%  p=0.6476\n"
             "  差 4.6pp，方向虽符合直觉，但两组各自都不显著。\n"
             "按品种拆后每组只剩 0~5 个样本（gold 薄 1/3、silver 薄 0/2、wti 薄 1/2、"
             "tlt 薄 1/1、qqq 无薄样本），完全无从判断。\n"
             "按信号方向取符号的 5 日收益：薄 −1.738%（n=8）vs 厚 −1.143%（n=19）。",
        caveat="⚠️ 要以 80% 功效检出 37.5% vs 42.1% 这 4.6pp 的差距，**每组需 n≈1,777**；"
               "按当前 8 品种约 0.7 次开火/交易日、其中约三成是薄分母估算，"
               "攒够薄组约需 **34 年**。samples_to_significance 对三组均返回 None"
               "（命中率太接近 50%，5000 样本内也达不到显著）。\n"
               "→ 结论：**不加硬门**。一个案例上的外部印证不足以改判定口径，"
               "否则就是拿单点观察当规则 —— 这正是手工台账被废弃的原因。\n"
               "→ 已做的是**展示层降级**（不需统计支持）：StrongSignal 增 counter_pressure，"
               "持仓冲突告警的摘要行带上分母与「分母过小·比值失真」标注。"
               "此前 ALERT 文件与桌面通知只写「⚡强看跌（491.5×）」，"
               "分母 813 只出现在详情页 reasons[0]，最该看见的地方反而看不见。"),
