...
③ 出场　只在【收盘越过卖腿】时平仓；换墙不平、浮盈不平
    ⛔ 2026-09-26 更正（Codex A01，Claude 复核成立）：下表来自旧 step3_exit，
    它用 D 日收盘判破卖腿、却按 D 日【盘前】快照报价平仓 —— 前视。
    表中数字与「砍掉亏损」的结论**全部作废**，只作错误证据保留。
    修后退出模型见 analyze/spread_exit.py，仍只是快照估价模型，未验证可成交。
    旧表（作废）—— 两侧对照（SLV 墙上 宽3档 DTE7，call=逆势 / put=顺势 / 合计）：
        持有到期      −329 / +249 / −80
        破卖腿即平     −80 / +249 / +169   ← 唯一两侧都不吃亏
        换墙即平      −302 / +113 / −189   ← 坏规则
        破墙或浮盈50%  −21 / +116 / +95
    （以上结论作废：前视。「破卖腿即平」规则本身保留为研究配置，其减损效果未知。）
...
（call 侧，样本期白银 +9.7%）实测是 7~8%。
⚠️ 「平衡破墙率」按两点极端损益（全收权利金 / 全亏宽度）算，实际破卖腿后常是部分亏损，
   两者不能直接比；且 50 笔零破墙的单侧 95% 上界仍 5.8%，点估计不能当通过线（Codex A15）。
（旧版此处写「破墙平仓砍掉 76% 亏损、拉回平衡线」—— 前视，已撤回。）
