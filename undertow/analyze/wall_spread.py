...
#    出候选，等于换个说法照做同一笔交易。
#    2026-09-02 更新：① ② ③ 都做完了，结论是【策略前提本身不成立】——
#    安慰剂对照（scripts/placebo_wall_value.py，登记于 wall_edge_vs_placebo）显示：
#        结构墙   n=79 未破墙 100% 缓冲 5.2% 收益率 +110.0%
#        非墙同距 n=79 未破墙 100% 缓冲 5.0% 收益率  +98.4%
#        37 个日期簇符号置换 p=0.267 —— 无法区分
#    即「墙难破」成立，但「卖在墙上因此更好」不成立：两组都 100% 未破墙，
#    难破来自【距离 5% + 样本期上涨】，墙没有贡献。
#    剥掉「墙」之后，剩下的是一个普通的卖远虚值 put 价差策略。
#
#    ⛔ 因此本模块不是"等条件满足就能重开"，而是**前提被证伪**。
#    要重开必须先回答：墙在什么条件下才有额外价值？（跌市？特定 OI 量级？
#    特定到期结构？）在有肯定答案之前，把它当"卖远虚值价差"来做即可，
#    不必也不应该绕道墙位。
ACTIVE: set[str] = set()
