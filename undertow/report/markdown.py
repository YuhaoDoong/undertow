...
FLOW_DISCLAIMER = (
    "> ⚠️ **启发式 & 代理提示**：复刻该方法论「逐行权价分买卖方」的读法——"
    "**OI增+IV升=买方抬价、OI增+IV降=卖方写权**。延迟数据无逐笔成交，买卖方为**IV方向代理**推断；"
    "「Delta修正ΔIV」是对该方法论的**原理化近似**（剔除现价移动沿偏斜的机械IV项），边界行可能与人工酌情判断不同。"
...
def render_flow(fa, display_name: str) -> str:
    ...
    # —— 日对日买卖方判定（核心，需两份快照）——
    if fa.prev_date:
        L.append("### 日对日买卖方判定（ΔOI × Delta修正IV → 买/卖方，复刻机构口径表）")
