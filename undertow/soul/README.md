...
| 文件 | 作用 |
|---|---|
| `journal.py` | **交易日记 + 事前判断**：`Thesis` 开仓前落盘方向/依据/时间预期/**失效条件**/目标，事后标对错；`thesis_stats()` 把【判断命中率】与【交易盈亏】**分开统计**并标出背离（判断对却亏钱＝问题在仓位不在判断力），样本<30 强制提示『不足以区分运气与能力』。`capture_trades()` 从券商成交+资金流水自动抓当日明细（含手续费）；JournalEntry 记录成交/账户变化/市场语境/起作用的规则/复盘/盖棺定论/**心情**。档案管『我该怎么做』，日记管『我实际做了什么、结果如何、当时什么心情』。 |
