...
class Exits:
    ...
    # —— 机器可读阈值 ——
    # 描述文字是给人看的，里面会出现多个金额（最大亏损、仓位占比、目标位…），
    # 靠正则从散文里抓第一个 $ 数会抓错——2026-08-26 实测把「最大亏损$90」当成了止损线。
    # 故阈值单列，单位一律【美元·整个持仓的可平仓价值】。留空表示该项无数值阈值。
    stop_value: float | None = None      # 跌到这个可平仓价值就平
    target_value: float | None = None    # 涨到这个可平仓价值就平
